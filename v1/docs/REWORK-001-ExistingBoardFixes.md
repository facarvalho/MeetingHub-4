...
Versão: 1.1 · Data: 2026-08-28
Aplica-se às 5 placas do lote JLCPCB `W2026080810364558` (gerbers v10.0).

**Estado do projeto que corresponde à placa física = commit `b86a034`.**
O HEAD atual (`eb61fb6`) já contém as correções desta sessão (esquemático + PCB
pré-preparado + footprint PS-22F03 + BOMs). Todas as coordenadas de furo citadas
aqui foram tiradas de `b86a034` (a placa que você tem na mão).
...
- `hardware/KiCad/MeetingHub-4-v1/MIXER.kicad_sch` — RV1–RV5: sinal e VBIAS
  trocados de terminal (netlist conferida: pino 1/4 = VBIAS, pino 3/6 = sinal,
  pino 2/5 = cursor).
- `hardware/KiCad/MeetingHub-4-v1/MICSW.kicad_sch` — K1–K4: bobina nos pinos 2 e 9
...
O **layout do PCB (`MeetingHub-4-v1.kicad_pcb`) NÃO foi re-roteado** — fazer isso à
...
## 1b. Caminho mínimo — só "microfone + sem segurar botão"

Se por agora você só quer o **mic funcionando** e **acabar com o "segurar
botão"** (deixando os potenciômetros / VBIAS para depois):

### Passo 1 — Microfone (Parte A)

Retrabalhar os relés **K1–K4** + diodos **D2–D5**. Por canal:

- levantar os pinos **1, 9, 10** do relé;
- jumper **pino 9 → furo 1**; jumper **pino 10 → furo 9**; pino 1 isolado;
- **D2–D5**: para o teste, o mais simples é **remover o diodo** (o relé funciona
  sem ele numa bancada). Para o definitivo, invertê-lo (faixa para o lado
  oposto ao relé).

Comece só por **K1 + D2**, teste o notebook 1, depois faça K2/K3/K4.

### Passo 2 — Tirar o MUTE (SW1)

Você autorizou tirar o mudo. Na placa, **SW1 fica em série com a linha do
microfone**. Solução: **pingo de solda ligando os 2 furos do SW1 que têm
trilha** — `HEADSET_MIC` (furo em ~(73,15 , 214,5)) com o furo que vai aos
relés (~(71,9 , 212,0)). O microfone passa a passar sempre. O "mudo" passa a
ser *nenhuma seletora ligada*.

### Passo 3 — Trocar SW2–SW5 por botão com trava

Para cada uma (SW2, SW3, SW4, SW5):

1. Desssolde a chave tátil momentânea.
2. Ligue um **botão self-locking (push-on/push-off)** com **2 fios**:
   - um fio → o furo do SW que tem a trilha de **+5V_AUDIO**
     (SW2 ~(96,8 , 214,5) · SW3 ~(109,5 , 214,5) · SW4 ~(123,8 , 214,5) ·
     SW5 ~(135,9 , 214,5));
   - outro fio → o furo com a trilha que vai ao **relé**
     (SW2 ~(95,6 , 212,0) · SW3 ~(108,3 , 212,0) · SW4 ~(122,5 , 212,0) ·
     SW5 ~(134,6 , 212,0)).
   - Confirme os furos com o multímetro (um tem +5 V; o outro tem continuidade
     até a bobina do relé correspondente **depois** do Passo 1).
3. Travado = aquele notebook selecionado. Solto = não. Todos soltos = mudo.

### Peça a comprar

| Qtd | Item | Onde |
|---|---|---|
| 4 (+1 reserva) | **Chave push com trava / "self-locking" / trava-destrava** — **PS-22F03 / A03** (6 pinos; usa COM + NO de um polo = 2 fios). É a mesma peça do respin. | LCSC **C2848947** (ou HOOYA **C19190927**); ou "chave A03 self-locking" em qualquer loja |
| — | *Alternativa mais fácil de montar:* **botão de painel com trava, 2 terminais** (self-locking 8–12 mm, tipo PBS-11B / R13-507). 2 lugs, monta no painel, 2 fios até a placa. | loja local / AliExpress |

**Alternativa com 1 peça só:** uma **chave rotativa 1 polo × 5 posições**
(OFF / NB1 / NB2 / NB3 / NB4). Comum → +5 V; cada posição → o furo "relé" de
SW2..SW5; OFF já é o mudo. Ver **Parte D.3**. Remove SW2–SW5 e ainda dá a ponte
no SW1.

**Seleção exclusiva com botões (comportamento de KVM — aperto um, o outro
desarma sozinho):** os botões continuam momentâneos + uma placa auxiliar
CD4043 + matriz de diodos faz a trava eletrônica. Esse caminho **substitui**
a Parte A + Parte D: ver **`docs/Architecture/SCH-010-OneHotMicSelector.md`**
(esquema completo, lista de materiais, ligação furo a furo).

> Detalhes completos: **Parte A** (relés) e **Parte D** (chaves) abaixo.
