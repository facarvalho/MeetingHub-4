# MeetingHub-4 — v1 (the fabricated design)

Analog audio hub for 4 laptops, with a single TRRS headset and microphone selection — no drivers, no firmware, no software.

> **v1** = the original 4-layer board (fabricated at JLCPCB as `MeetingHub-4`,
> lot `W2026080810364558`, gerber rev v10.0). The first article revealed 3
> schematic bugs (pots reversed, mic relays mis-wired, VBIAS unbuffered) — see
> `docs/REWORK-001…004`. The **[v2](../v2/)** folder is the 2-layer, all-THT
> respin that fixes them and adds the on-board one-hot mic selector.
> The KiCad project files were renamed `MeetingHub-4` → `MeetingHub-4-v1` in the
> 2026-09-01 repo restructure; the frozen fab outputs in `hardware/Gerbers/`
> keep their as-shipped names.

| | |
|---|---|
| **Status** | ⚠️ **PCB mid-rework, not ready for fabrication** — see [HANDOFF-2row-pots.md](hardware/KiCad/MeetingHub-4-v1/HANDOFF-2row-pots.md) for what's left. Gerbers in `hardware/Gerbers/` are stale (from the previous, fully-validated 265.1x160.1mm version). |
...
hardware/
  KiCad/MeetingHub-4-v1/     Complete KiCad project (schematic + PCB, source of truth)
