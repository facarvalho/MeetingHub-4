...
**Selector-behaviour decision:** the user wants KVM-style **exclusive one-hot**
selection (press NB2 → NB1 releases itself), with momentary buttons. All-
hardware design added in `docs/Architecture/SCH-010-OneHotMicSelector.md`:
CD4043B quad RS latch + 16-diode matrix + power-on reset + 4x 2N7000 low-side
coil drivers. This supersedes REWORK-001 Parts A+D for the one-hot build (relay
coil re-wired pin2=+5V / pin9=MOSFET drain, D2-D5 flipped cathode->pin2). SW1
becomes a bridge or an optional 5th "mute/reset-all" button. Respin needs a new
SELECT_LOGIC schematic sheet (not built yet). The PS-22F03 latching-button work
below stays valid only if the plain-latching route is chosen instead.
...
PCB (`MeetingHub-4-v1.kicad_pcb`) **pre-staged by script** (no manual routing yet):
...
errors. Full list in `hardware/KiCad/MeetingHub-4-v1/DRC.rpt` (local,
...
- Applied to `MeetingHub-4-v1.kicad_pcb` (same position/rotation, nets
  ...
  entry below).
- BOM-MeetingHub-4-v1.csv, BOM-PCBA-MeetingHub-4-v1.csv, and
...
- New custom footprint `SW_SHOUHAN_TS665ZJ`, added to a new project-local
  library (`hardware/KiCad/MeetingHub-4-v1/MeetingHub-4.pretty/` +
...
- Applied to `MeetingHub-4-v1.kicad_pcb` (all 5 footprint instances
