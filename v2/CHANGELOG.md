...
## 2026-09-05 — v2 rev-11 (J2–J6 footprint rebuilt — JLC SMT DFM reject)

**Problem:** JLC's SMT/assembly team rejected the (already fabricated) v2 order —
the package of C136687 (Korean Hroparts PJ-3200B-4A) *"cannot match the
corresponding pads on the PCB"* for J2–J6. Confirmed: the rev-8 footprint
`Jack_3.5mm_PJ-3200B-4A_Horizontal` was hand-drawn from the paper datasheet and
never checked against JLC's own land pattern. Vs the real C136687 pattern (pulled
from the LCEDA/EasyEDA library, `easyeda2kicad` v1.0.1):

| feature | rev-8 (wrong) | real C136687 |
|---|---|---|
| T–R1 / R1–R2 pad pitch | 4.00 / 3.00 mm | 4.00 / **3.34** mm |
| Sleeve pad vs T | −4.6 mm across, +1.1 mm | **+1.0 mm across, +4.4 mm** (~6.5 mm out of place) |
| locating posts | Ø1.30, **4.6 mm apart**, beside R1/R2 | **Ø1.50, 7.0 mm apart**, straddling the row |
| pad hole | oval 1.2×0.7 | **slot 0.9×1.8 mm** |

The Sleeve pin and both locating posts landed nowhere near their holes → the part
could not be assembled by JLC *or* by hand. The fabricated boards from that order
are scrap for this jack.

**Fix — footprint only, geometry rebuilt from JLC's library data:**

- `MeetingHub-4-v2.pretty/Jack_3.5mm_PJ-3200B-4A_Horizontal.kicad_mod` rewritten:
  4 slotted pads (0.9×1.8 mm) T/R1/R2 in a row at 4.00 / 3.34 mm + offset Sleeve,
  2× Ø1.50 NPTH posts 7.0 mm apart, exactly the LCEDA land pattern, rotated +90°
  so the barrel still points −Y (KiCad rot 0 = barrel over the −Y edge, same as
  rev-8, so J2–J5 stay rot 0 / J6 rot 180).
- **Pin→function unchanged** (T=LCEDA pin 2, R1=pin 4, R2=pin 3, S=pin 1). This
  matches the rev-8 datasheet call *and* is corroborated by the 4 mm-gap-end = T
  convention in KiCad's own `Jack_3.5mm_KoreanHropartsElec_PJ-320D-4A` (same
  manufacturer) and `Jack_3.5mm_PJ320E`. **Bench-confirm** anyway: plug a 3.5 mm
  TRRS plug, check continuity plug-sleeve→S pad, plug-tip→T pad.
- 3D model: `MeetingHub-4-v2.3dshapes/Jack_3.5mm_PJ-3200B-4A_Horizontal.wrl`
  (+ `.step`), generated by `easyeda2kicad` from C136687.
- `scripts/pcb.py`: J2–J5 y = REAR+10.2 → **REAR+8.0**, J6 FRONT−10.2 → FRONT−8.0
  (the rebuilt body is 2.85 mm shorter toward −Y); GND zone outline chamfered to
  the rounded rect + 4 pre-placed GND corner stitching vias (the re-route
  otherwise leaves one GND pour lobe near a corner unbonded → 1 "unconnected").
- `scripts/route.py` added (2-layer freerouting driver; v2 had none).
- `scripts/post.py`: also hides each footprint's F.Fab value text (KiCad-script
  default = the footprint name) — it was cluttering assembly renders. The
  manufactured silkscreen only ever had the reference designators; unchanged.
- **Netlist connectivity unchanged** (jack pins are still S/T/R1/R2 by name).
  Re-routed (freerouting 2.2.4, 0 unrouted), ses_import, post.
- **Headless DRC with the project loaded: 0 errors, 0 unconnected, 0 footprint
  errors, 5 cosmetic `silk_over_copper` (RV1–5 refs) — identical to rev-10.**
  0 SMD pads. Gerbers / drill / `-Gerbers.zip` / CPL / both BOMs regenerated.
- CPL J2–J5 rot 90 / J6 rot 270 (bomcpl +90° correction, same as rev-9/10).
  **Verify the jack rotation in JLC's assembly preview before re-ordering.**

**Order action:** the current PCB+SMT order is unrecoverable for the jacks →
reply **C (cancel both)** to JLC and re-upload this (rev-11) gerber + BOM + CPL.

**Still owed before re-order (GUI / mechanical, unchanged from rev-10 + new):**
open in the KiCad GUI, native ERC/DRC, 3D view — check the PJ-3200B-4A barrel
overhang (~1.4 mm) and the Ø1.5 post-to-edge (~1.5 mm) vs the acrylic; update the
enclosure cut plan for the new jack cut-outs; bench continuity-check the jack
pinout; the 3 rev-10 bench items (RK097 taper, R16/R20 code, de-selected-mic OS).
